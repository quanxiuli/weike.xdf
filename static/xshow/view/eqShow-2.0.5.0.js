				...
				d = function(a, d, e) {
					...
					if ($("textarea", e).each(function() {
						...
						return h ? void $.ajax({
							cache: !0,
							type: "POST",
							url: PREFIX_S1_URL + "ajax_scenedataadd?id=" + d,
							data: $.param(g),
							async: !1,
							error: function(a) {
								alert("Connection error")
...
function(a, b) {
	...
	function eqFreepage(a, b, c) {
		...
		if (ad = 1, a.obj.property.lastPageId) customLastPage = !0, $.ajax({
			...
			success: function(c) {
				...
				for (var m = 0; m < c.obj.elements.length; m++)"2" == c.obj.elements[m].type && /http:\/\/www.xdf.cn/.test(c.obj.elements[m].content) && (c.obj.elements[m].content = c.obj.elements[m].content.replace(/;url=.*com"/, ";url=" + encodeURIComponent(redirectUrl) + '"'));
				d(c.obj, b, a)
			}
		});
		else {
		...
		ad = 2, $.ajax({
			...
			success: function(c) {
				...
				for (d.length; f < d.length; f++) {
					var g = d[f];
					if (g.sceneId = a.obj.id, g.pageId = b[b.length - 1].id, a.obj.property.bottomLabel.name && a.obj.property.bottomLabel.url && "http://" != a.obj.property.bottomLabel.url) 2 == g.type && g.content.indexOf("科技公司") > 0 && (g.content = g.content.replace(/科技公司/, '<a href="' + PREFIX_S1_URL + "?id=" + a.obj.id + "&amp;url=" + encodeURIComponent(a.obj.property.bottomLabel.url) + '" target=_blank>' + a.obj.property.bottomLabel.name + "</a>"));
	...
	function parsePage(a, b) {
		...
		for (var k = 1; k <= a.length; k++) {
			...
			if (k == a.length && (isMobile || $("img").on("dragstart", function(a) {
				...
				var l = PREFIX_S1_URL + "ajax_sceneaddpv?id=" + b.obj.id;
	...
	window.appendLastPage = function(a, b, c, d, e, f) {
		if (pageMode = c, preview = d, param = e, ad = f, 100 == a.obj.bizType ? (redirectUrl = "https://itunes.apple.com/us/app/easyshow-free-h5-sence-slides/id987351120?mt=8", companyName = "Hypefolio") : (redirectUrl = "http://xdf.cn", companyName = "技术支持"), appendActivityPage(a, b), a.obj.createTime > 14165028e5) if (100 == a.obj.bizType) eqFreepage(a, b);
	...
	function parseJson(a, b) {
		...
		var c = [];
		return a.obj.property && (a.obj.property = JSON.parse(a.obj.property) || {}), a.obj.bgAudio && "string" == typeof a.obj.bgAudio && (a.obj.bgAudio = JSON.parse(a.obj.bgAudio) || null), c = a.list, c.length <= 0 ? (alert("此场景不存在!"), void(0)) : void appendLastPage(a, c, pageMode, preview, param, ad) //window.location.href = "http://.com"
	}
	...
	a.getScript("/public/xshow/view/js/d.js", function() {
		function a() {
