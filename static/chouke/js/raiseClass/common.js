...
$('.btn-down-container,.btn-download,#popupSuccessWechat .open,.open-app,.add-down-app .button').on('touchend',function(){
    window.location.href = 'http://a.app.qq.com/o/simple.jsp?pkgname=com.xdf.pocket';
    //window.location.href = 'http://app.xdf.cn/app/phone/index.html';
    return false
})
//通用区分app 微信
if(inApp()){
...
var indexFn = function (begin) {
                ...
                if (json.result == true) {
                    ...
                    $(document).on('touchend','#askFriend',function(){
                        if(inApp()){
                            ...
                            var sUrl=Global.wechatUrl+'raiseClassNew/index.html?activityId='+activityId;
                    ...
                    if(inApp()){
                        ...
                        weChatMsg={
                            ...
                            linkVal:'https://open.weixin.qq.com/connect/oauth2/authorize?appid='+Global.appid+'&redirect_uri='+'http://'+Global.redirect_uri+'/xdfhome/raiseClassNew/index.html?activityId='+getRequest()['activityId']+'&response_type=code&scope=snsapi_userinfo&state='+getRequest()['userId']+'#wechat_redirect',
                    ...
                    success: function (json) {
                        if(json.result==true){
                            if(json.data.Status==1){//成功 开始发起筹课
                                Global.userId = JSON.parse(json.data.Data).UserId;
                                Global.mobile = JSON.parse(json.data.Data).Mobile;
                                startFn();
                            }else{
    ...
    $('.yanzm2').on('touchend',function(){
        ...
        jQuery.ajax({
            ...
            success: function (json) {
                if(json.result==true){
                    if(JSON.parse(json.data).Status==1){
                        layer.msg('验证码发送成功！');
                        var countdown=60;
                        function settimeFn() {
                            var cdDom=$('.yanzm2');
    ...
    jQuery.ajax({
        ...
        success: function (json) {
            if(json.result==true&&json.data==true){//发布过 进入页面
                location.href='newRaise.html?activityId='+activityId+'&userId='+Global.userId;
            }else{//未发布过 开始发布
                var businessP = {
                    "channel":"Wechat",
                    "activityId":activityId,
                    ...
                    success: function (json) {
                        if (json.result == true) {//发起成功!  您已经在手机应用发起过该活动了哦  您已经在微信发起过该活动了哦
                            if(json.message=='发起成功!'){
                                location.href='newRaise.html?activityId='+activityId+'&userId='+Global.userId+'&first=1';
                            }else{
                                location.href='newRaise.html?activityId='+activityId+'&userId='+Global.userId;
                            }
    ...
    jQuery.ajax({
        ...
        success: function (json) {
            if (json.result == true) {
                ...
                $('.ck-user .info .name').html(decodeURI(decodeURI(json.data.activity.userName)));
                ...
                weChatMsg={
                    ...
                    linkVal: 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='+Global.appid+'&redirect_uri='+'http://'+Global.redirect_uri+'/xdfhome/raiseClassNew/helpRaise.html?activityId='+getRequest()['activityId']+'&response_type=code&scope=snsapi_userinfo&state='+userId+'#wechat_redirect',
                ...
                if (json.data.activity.isSuccess == 1) {//成功了
                    ...
                    if(inApp()){
                        ...
                        weChatMsg={
                            titleVal: json.data.activity.name,
                            descVal:json.data.activity.content,
                            linkVal: 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='+Global.appid+'&redirect_uri='+'http://'+Global.redirect_uri+'/xdfhome/raiseClassNew/index.html?activityId='+getRequest()['activityId']+'&response_type=code&scope=snsapi_userinfo&state='+userId+'#wechat_redirect',
                            imgUrlVal:json.data.activity.wechatImgUrl
                        }
                ...
                if(inApp()){
                    ...
                    if(json.data.activity.isSuccess == 1){
                        $('#askFriend,.askHelp').on('touchend',function(){
                            ...
                            var sUrl='https://open.weixin.qq.com/connect/oauth2/authorize?appid='+Global.appid+'&redirect_uri='+'http://'+Global.redirect_uri+'/xdfhome/raiseClassNew/index.html?activityId='+getRequest()['activityId']+'&response_type=code&scope=snsapi_userinfo&state='+userId+'#wechat_redirect'
                        ...
                        $('#askFriend,.askHelp').on('touchend',function(){
                            var sName=json.data.activity.name;
                            var sDescribe=json.data.activity.content;
                            var sUrl='https://open.weixin.qq.com/connect/oauth2/authorize?appid='+Global.appid+'&redirect_uri='+'http://'+Global.redirect_uri+'/xdfhome/raiseClassNew/helpRaise.html?activityId='+getRequest()['activityId']+'&response_type=code&scope=snsapi_userinfo&state='+userId+'#wechat_redirect'
                            var sImgUrl = json.data.activity.wechatImgUrl;
                            shareFn(sName,sDescribe,sUrl,sImgUrl);
                            return false
                        })
...
function helpRaiseFn() {
            ...
            if (json.result == true) {
                ...
                $(document).on('touchend','#askFriend',function(){
                    ...
                    jQuery.ajax({
                        ...
                        success: function (json) {
                            if (json.result == true) {
                                window.location.href='http://'+Global.redirect_uri+'/xdfhome/raiseClassNew/helpRaiseSuccess.html?name='+userName+'&classMoney='+classMoney+'&data='+json.data+'&activityId='+activityId+'&userId='+userId;
                            }else{//失败提示
                                layer.msg(json.message)
                            }
                        }
    ...
    $('#goIndex').on('touchend', function () {
        window.location.href='https://open.weixin.qq.com/connect/oauth2/authorize?appid='+Global.appid+'&redirect_uri='+'http://'+Global.redirect_uri+'/xdfhome/raiseClassNew/index.html?activityId='+getRequest()['activityId']+'&response_type=code&scope=snsapi_userinfo&state='+userId+'#wechat_redirect';
        return false
    })
}
