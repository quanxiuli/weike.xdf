    ...
    jQuery.ajax({
        ...
        success: function (json) {
            if (json.result == true) {
                ...
                var helpWealSwitch = activity.helpWealSwitch;
                if(helpWealSwitch==1){
                	$('#helpWealSwitch').prop('checked',true);
                	$('#wealName').val(activity.wealName);
					$('#wealRemark').val(activity.wealRemark);
					$('#wealBootIntro').val(activity.wealBootIntro);
					$('#wealIntroduce').val(activity.wealIntroduce);
                }else{
                	$('#helpWealSwitch').prop('checked',false);
                	$('#helpBatchDiv').hide();
                	$('#wealName').val('');
					$('#wealRemark').val('');
					$('#wealBootIntro').val('');
                ...
                setTextSize_cknew(helpWealSwitch);
            } else {
...
function setTextSize_cknew(helpWealSwitch) {
    //编辑页面使用
    ...
    if(helpWealSwitch){
    	$('#explainWealNameMinCnt').text($('#wealName').val().length);
    	$('#explainWealRemarkMinCnt').text($('#wealRemark').val().length);
    	$('#explainWealBootCnt').text($('#wealBootIntro').val().length);
    	$('#explainWealIntroduceCnt').text($('#wealIntroduce').val().length);
    }
}
function getRequest() {
