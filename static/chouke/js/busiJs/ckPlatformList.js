...
function createCk() {
    if (currentAreaId == "") {//$("#areaIdFinal").val() == ""
        ...
        if(currentType == 1){//筹课抽奖
            ...
            changeCenter('/chouke/setcourseinfo?publish=' + publish);
        }else if(currentType == 2){//筹课
            //暂时没有权限管理
            publish = true;
            changeCenter('/chouke/setraiseclassinfo?publish=' + publish);
...
function ckEdit(id,areaId) {
    if (areaId == "") {
        ...
        if(currentType == 1){
            window.location.href = '/chouke/updatecourse?ckId=' + id;
        }else if(currentType == 2){
            window.location.href = '/chouke/updateraiseclassinfo?ckId=' + id;
