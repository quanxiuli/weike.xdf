...
function save(step, isEmpty) {
    ...
    $('.disableCss').removeAttr('onclick');
    var d = constructionParams(rsaEncryptedString(businessP), "c4248b1e72c44b4ebb2bf51842292ace");
    jQuery.ajax({
        type: "POST",
        url: Global.actionURL,
        async: true,
        ...
        success: function (json) {
            if (json.result == true) {
                ...
                changeCenter("./ckMain.html?type=1");
...
function saveEdit(step, isEmpty, statusvalue) {
    ...
    var d = constructionParams(rsaEncryptedString(businessP), "8bd6b2bb821b41a4b535ea63b5ab7622");
    jQuery.ajax({
        type: "POST",
        url: Global.actionURL,
        async: false,
        ...
        success: function (json) {
            if (json.result == true) {
                ...
                if(statusvalue == 0){
                    layer.mstatusvaluesg("保存成功!", {icon: 6});
